"""
pytest-Tests für die GUI-freie Logik in core.py.

Lauf:  pytest test_core.py        (oder:  python test_core.py)
core.py hat keine tkinter-Abhaengigkeit und ist daher direkt importierbar.
"""
...
def test_sanitize_sheet_name():
    ...
    used = set()
    a = core.sanitize_sheet_name("Mappe/1", used)
    check(a == "Mappe_1", f"ungültige Zeichen ersetzt -> {a!r}")
    b = core.sanitize_sheet_name("Mappe/1", used)
    check(b != a and b.startswith("Mappe_1"), f"Duplikat eindeutig gemacht -> {b!r}")
    long = core.sanitize_sheet_name("X" * 50, used)
    check(len(long) <= 31, f"max. 31 Zeichen -> len={len(long)}")


def test_escape_excel_formula():
    print("test_escape_excel_formula")
    check(core.escape_excel_formula("=SUM(A1:A9)") == "'=SUM(A1:A9)", "Formel (=) entschärft")
    check(core.escape_excel_formula("+1+1") == "'+1+1", "Formel (+) entschärft")
    check(core.escape_excel_formula("@cmd") == "'@cmd", "Formel (@) entschärft")
    check(core.escape_excel_formula("-1-1") == "'-1-1", "Formel (-) entschärft")
    check(core.escape_excel_formula("ART-1") == "ART-1", "harmloser Wert unverändert")
    check(core.escape_excel_formula("Widget") == "Widget", "Text unverändert")
...
def test_parse_edifact():
    ...
    header, items, doc_id = core.parse_edifact(raw)
...
def test_parse_orders05_xml():
    ...
    header, items, doc_id = core.parse_orders05_xml(raw)
...
def test_parse_opentrans_xml():
    ...
    header, items, doc_id = core.parse_opentrans_xml(raw)
    check(doc_id == "4711", f"Dokument-ID -> {doc_id!r}")
    check(header.get("Währung") == "EUR", f"Währung -> {header.get('Währung')!r}")
    check(len(items) == 1, f"genau eine Position -> {len(items)}")
    it = items[0]
...
def test_parse_parameter_xml():
    ...
    h, v = core.parse_parameter_xml(wrapped)
    ...
    h2, v2 = core.parse_parameter_xml(frag)
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(path, "w", encoding="utf-8", newline="") as f:
            ...
            for i in range(5):
                f.write(f"{i},{i*2}\n")
        logs = []
        count = core.split_csv_file(path, chunk_size=2, log_fn=logs.append)
...
def test_strip_protection_xml():
    ...
    out = core._strip_protection_xml(xml).decode("utf-8")
...
def test_entferne_schutz_integration():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        ok, out = core.entferne_schutz_on_file(path)
...
def test_convert_files_to_workbook():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        errors, progress = [], []
        written = core.convert_files_to_workbook(
...
def test_convert_empty_creates_placeholder():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        written = core.convert_files_to_workbook([bad], out)
